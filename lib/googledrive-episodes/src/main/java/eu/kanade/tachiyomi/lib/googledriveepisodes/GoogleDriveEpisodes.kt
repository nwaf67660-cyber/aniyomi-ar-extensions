...
class GoogleDriveEpisodes(private val client: OkHttpClient, private val headers: Headers) {
    ...
    fun getEpisodesFromFolder(folderId: String, path: String, maxRecDepth: Int, trimNames: Boolean): List<SEpisode> {
        ...
        fun traverseFolder(folderId: String, path: String, recursionDepth: Int = 0) {
            ...
            // FIX: كان "drivescript").first" والصح "driveDocument.select("script").first"
            ...
            while (pageToken != null) {
                ...
                parsed.items.forEachIndexed { index, item ->
                    if (item.mimeType.startsWith("video")) {
                        ...
                        episodeList.add(
                            SEpisode.create().apply {
                                name = if (trimNames) item.title.trimInfo() else item.title
                                url = "https://drive.google.com/uc?id=${item.id}"
                                episode_number = ITEM_NUMBER_REGEX.find(item.title.trimInfo())?.groupValues?.get(1)?.toFloatOrNull() ?: index.toFloat()
                                date_upload = -1L
                    ...
                    if (item.mimeType.endsWith(".folder")) {
                        traverseFolder(item.id, "$path/${item.title}", recursionDepth + 1)
                    }
                }

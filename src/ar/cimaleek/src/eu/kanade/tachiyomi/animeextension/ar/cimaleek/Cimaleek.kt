...
class Cimaleek : ConfigurableAnimeSource, ParsedAnimeHttpSource() {
        ...
        for (element in serverElements) {
            try {
                ...
                if (videos.isNotEmpty()) {
                    return videos.sortedWith(compareBy { it.quality.contains(preferredQuality()) }).reversed()
                }
            } catch (e: Exception) {
                println("Server extraction failed, trying next server: ${e.message}")
            }
        }
        return emptyList()
    }
    ...
    private fun tryExtractFromServer(element: Element, version: String, refererHeaders: Headers): List<Video> {
        ...
        val videoFrameBody: String = try {
            client.newCall(GET(videoUrl, refererHeaders)).execute().use { resp ->
                if (!resp.isSuccessful) throw IOException("Failed to fetch video frame: ${resp.code}")
                resp.body?.string() ?: ""
            }
        } catch (e: Exception) {
            println("Error requesting videoFrame: ${e.message}")
            return emptyList()
        }
        ...
        return when {
            resolved.contains(".mp4") -> listOf(Video(resolved, element.text(), resolved, headers = refererHeaders))
            resolved.contains(".m3u8") -> {
                ...
                if (finalUrl.contains(".mp4")) listOf(Video(finalUrl, element.text(), finalUrl, headers = refererHeaders))
                else if (finalUrl.contains(".m3u8")) {
                    val subtitleList = if (subtitle.isNotBlank()) listOf(Track(subtitle, "Arabic")) else emptyList()
                    try { playlistUtils.extractFromHls(finalUrl, videoNameGen = { "${element.text()}: $it" }, subtitleList = subtitleList) } catch (e: Exception) { emptyList() }
                } else emptyList()
            }
        }
    }
    ...
    private fun extractEmbedUrl(body: String): String? {
        ...
        if (jsonCandidate.isNotEmpty()) return jsonCandidate.replace("\\/", "/")
    ...
    private fun resolveEmbedUrl(embedUrl: String, refererHeaders: Headers): String {
        val resolved = try {
            when {
                ...
                embedUrl.startsWith("/") -> baseUrl.toHttpUrl().newBuilder().encodedPath(embedUrl).build().toString()
                else -> embedUrl
            }
        } catch (e: Exception) { embedUrl }
        return try {
            client.newCall(GET(resolved, refererHeaders)).execute().use { resp -> resp.request.url.toString() }
        } catch (e: Exception) { resolved }
    }
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        return if (query.isNotBlank()) {
            ...
            url.addPathSegment("page").addPathSegment("$page")
            if (categoryFilter.state != 0) url.addQueryParameter("type", categoryFilter.toUriPart())
    ...
    private class GenreFilter : SingleFilter(
        ...
        arrayOf(
            ...
            "Science-fiction","Romance","Comedy","Family","Drama","Thriller","Crime","Horror"
        ).sortedArray()
    )

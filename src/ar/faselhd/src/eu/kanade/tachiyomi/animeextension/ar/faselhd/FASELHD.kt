...
class FASELHD : ConfigurableAnimeSource, ParsedAnimeHttpSource() {
    ...
    private fun createRequestWithCloudflareBypass(url: String): Request {
        return GET(url, cfHeaders)
    ...
    private fun bypassCloudflare(url: String): Document? {
        return try {
            // Step 1: Initial request to get CF challenge
            val initialResponse = client.newCall(createRequestWithCloudflareBypass(url)).execute()
            val initialDoc = initialResponse.asJsoup()
            
            // Check if Cloudflare challenge page
            if (initialDoc.select("title").any { it.text().contains("Checking your browser") || it.text().contains("Just a moment") }) {
                // Wait and retry with JS challenge simulation
                Thread.sleep(5000) // Wait for CF to clear
                
                // Step 2: Retry with enhanced headers
                ...
                val retryRequest = client.newCall(
                    GET(url, retryHeaders)
                        .cacheControl(CacheControl.Builder().noCache().build())
                        .build()
                ).execute()
                
                retryRequest.use { response ->
    ...
    override fun popularAnimeRequest(page: Int): Request = createRequestWithCloudflareBypass("$baseUrl/anime/page/$page")
            ...
            if (doc.select(episodeListSelector()).isNullOrEmpty()) {
                ...
                doc.selectFirst(seasonsNextPageSelector(seasonNumber))?.let {
                    ...
                    val seasonUrl = "$baseUrl/?p=" + it.select("div.seasonDiv")
                        ...
                        .substringBeforeLast("'")
                    val seasonDoc = bypassCloudflare(seasonUrl) ?: client.newCall(
                        createRequestWithCloudflareBypass(seasonUrl)
                    ).execute().asJsoup()
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        return document.select(videoListSelector()).parallelCatchingFlatMapBlocking { element ->
            ...
            val videoDoc = bypassCloudflare(url) ?: client.newCall(createRequestWithCloudflareBypass(url)).execute().asJsoup()
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        return if (query.isNotBlank()) {
            createRequestWithCloudflareBypass("$baseUrl/page/$page?s=$query")
        } else {
            ...
            url.addPathSegment("$page")
            createRequestWithCloudflareBypass(url.toString())
        }
    }
    ...
    override fun latestUpdatesRequest(page: Int): Request = createRequestWithCloudflareBypass("$baseUrl/most_recent/page/$page")
